...
-   **Check Dependencies**: Check if `google-antigravity` is listed in the
    project's dependencies (e.g., `package.json`, `package.json`).
-   **Install Package**: Ensure the `google-antigravity` Node.js package is
    installed.
...
-   If the user needs to equip an agent with custom capabilities (tools) derived
    from TypeScript functions, or maintain agent state across tool execution, read
...
-   If the user needs an agent to output structured data (e.g., JSON matching a
    JSON schema) for reliable integration, read
