#!/bin/bash
# install_localharness.sh 
# Download prebuild localharness binary
...
# Detect operating system and architecture for binary compatibility
case "$(uname -s)" in
  Darwin)
    case "$(uname -m)" in
...
# Validate required host CLI utilities are present
...
# 1. Attempt to query the PyPI JSON API to locate the download URL for the target platform wheel
...
# 2. Fallback to querying the PyPI Simple Repository HTML index if the JSON API is unavailable
...
# 3. Last resort fallback to a hardcoded URL for the default version if PyPI querying fails entirely
if [ -z "$DOWNLOAD_URL" ]; then
...
# Use a secure isolated temporary directory to prevent pollution of the working directory on failure or abort
TMP_DIR=$(mktemp -d)
trap 'rm -rf "$TMP_DIR"' EXIT

WHEEL_FILE="${TMP_DIR}/package.whl"

# Download the wheel package to the temporary directory
echo "Downloading wheel package..."
curl -# -L -o "${WHEEL_FILE}" "${DOWNLOAD_URL}"

# Extract only the localharness binary into the temporary directory
echo "Extracting localharness binary..."
unzip -q -o "${WHEEL_FILE}" "google/antigravity/bin/localharness" -d "${TMP_DIR}"

# Ensure destination directory exists and move the binary to its final location
mkdir -p bin
mv "${TMP_DIR}/google/antigravity/bin/localharness" bin/localharness
